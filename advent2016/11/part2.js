...
const parts = [ 'CG', 'CM', 'PG', 'PM', 'RG', 'RM', 'TG', 'TM', 'XG', 'XM', 'EG', 'EM', 'DG', 'DM' ];
...
while (queue.length) {
  ...
  do {
    do {
      ...
      const testState = applyMove(thisState, nextMove);
    } while(!(isValidMove(thisState, nextMove) &&
      JSON.stringify(testState) != thisState.previous &&
      isValidState(testState)
      )
    );
    ...
    if (nextMove) {
      const nextState = {
        ...
        previous: JSON.stringify(this.state.current),
        current: applyMove(thisState, nextMove)
      };
      queue.push(nextState);
      history[JSON.stringify(nextState.current)] = true;
